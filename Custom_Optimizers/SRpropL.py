...
def lr_update(params: List[Tensor],
    ...
    #print("SHAPE1", len(step_sizes), len(step_sizes[0]),len(step_sizes[1]),len(step_sizes[2]),len(step_sizes[3]),len(step_sizes[4]),len(step_sizes[5]))

    #print("SHAPE2", len(step_sizes), len(step_sizes[0]),len(step_sizes[1]),len(step_sizes[2]),len(step_sizes[3]),len(step_sizes[4]),len(step_sizes[5]))
...
class SRPROPL(Optimizer):
    def __init__(self, params, M=1, L=2, lr=1e-2, etas=(0.5, 1.2), lr_limits=(1e-6, 50), weight_decay=0, 
                 L_update_type = 'linear', L_update_step = '1 step', max_L = 50000, min_L = 0, s_plus = 1, 
                 s_min = -1, *, track_lr: bool = False, differentiable: bool = False):
            if not 0.0 <= lr:
        ...
        #### super() makes the class inherit properties from PyTorch's Optimizer class
            super(SRPROPL, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(SRPROPL, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if self.data_tally % self.next_L[-1] == 0 and self.lr_counter == 1:  # Second iteration of lr-update
                ...
                # Ensure step_sizes is initialized for all parameters
                if len(self.step_sizes) < len(group["params"]):
                    #print(f"⚠️ Fixing step_sizes mismatch: {len(self.step_sizes)} → {len(group['params'])}")
        ...
        #print(f"Current learning rates (step_sizes): {[s.mean().item() for s in self.step_sizes]}")          
        return loss