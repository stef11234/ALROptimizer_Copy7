...
class ADAMUPDL(Optimizer):
    def __init__(self, params, lr: Union[float, Tensor] = 1e-3, M=1, L=1, betas: Tuple[float, float] = (0.9, 0.999),
                 ...
                 track_lr: bool = False, differentiable: bool = False, L_update_type = 'linear', L_update_step = 'round up', 
                 max_L = 50000, min_L = 0, s_plus = 1, s_min = -1):
        if not 0.0 <= lr:
    ...
    #### Giving the class attributes that can be accessed later to update learning rates
        ...
        self.L = L
        self.M = M
        self.next_L = [0] #List of iterations when L is updated
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            #L, M = group["L"], group["M"]  # Use L and M hyperparameters, original
            ...
            if self.data_tally % self.next_L[-1] == 0 and self.lr_counter == 1:  # Second iteration of lr-update
                ...
                #print(self.weights3)
                w3 = torch.cat([x.flatten() for x in self.weights3])
                #print(len(w3),w3)
                self.lr_counter += 1
                self.L_sizes.append(self.L)
                self.L = self.L_update(M)
